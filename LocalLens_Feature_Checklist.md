...
- [x] **Profile Identity Fields & Stale-DB Migration Fix (2026-08-17)**: Resolved `sqlite3.OperationalError: no such column: users.display_name` (500 on `GET /api/v1/feed`) caused by the dev DB being behind migration `a1b2c3d4e5f6` (`add profile identity fields to users` — `display_name`, `username`, `date_of_birth`, `photo_url` + `ix_users_username`). Root cause: `make backend` started uvicorn without running pending Alembic migrations. Fix: applied `uv run alembic upgrade head` to the live dev DB (now at head) and hardened `make backend` to auto-run `alembic upgrade head` before launching the server; feed/profile endpoints verified returning 200. Troubleshooting note added to `README.md`.
- [ ] **Not started / pending future increments**: Production SMS provider & APNS/FCM push dispatchers, refresh token & secure keychain/keystore storage, auto-suggest merge flow for near-duplicates, mini-map interactive pin lock in compose, live ticking countdown renderers on escalation ladder, dispute photo attachment & 48h double-claim cooldown, pin density supercluster SDK, conflict-resolution sync strategy, image vector similarity for duplicate media, PII legal process ceremony, and backend auto-translation queue.
...
- [x] Implement global ThemeProvider (Light Mode, Dark Mode, System Auto). *(Provider + theme applied; user-facing selector available in Profile → Settings via F-13.)*
...
- [x] Build Camera Controls (Shutter `Key('shutterButton')`, Flip `Key('cameraFlipButton')`, Flash `Key('flashToggleButton')`, GPS lock `Key('gpsLockStatus')`). *(Real CameraController + CameraPreview using the `camera` package; GPS lock via `geolocator`; flash mode applied to controller; front/back flip reinitializes controller; all Keys preserved.)*
