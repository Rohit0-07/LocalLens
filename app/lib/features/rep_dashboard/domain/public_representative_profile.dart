/// Public representative performance profile.
///
/// Mirrors the backend `PublicRepresentativeProfileOut` (snake_case JSON keys).
/// Readable without authentication; `null` from the repository when the user
/// is not a representative (HTTP 404).
class PublicRepresentativeProfile {
  final String id;
  final int userId;
  ...
  const PublicRepresentativeProfile({
    required this.id,
    ...
    required this.ward,
    this.verifiedAt,
    this.totalWardIssues = 0,
    this.escalatedWardIssues = 0,
    this.respondedWardIssues = 0,
    this.pendingResponseWardIssues = 0,
  ...
  factory PublicRepresentativeProfile.fromJson(Map<String, dynamic> json) {
    return PublicRepresentativeProfile(
      ...
      verifiedAt: json['verified_at'] != null
          ...
          : null,
      totalWardIssues: (json['total_ward_issues'] as num?)?.toInt() ?? 0,
      escalatedWardIssues: (json['escalated_ward_issues'] as num?)?.toInt() ?? 0,
      respondedWardIssues: (json['responded_ward_issues'] as num?)?.toInt() ?? 0,
      pendingResponseWardIssues:
