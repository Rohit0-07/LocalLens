...
void main() {
  group('F-13: User Profile, Settings & Localization UX Tests', () {
    ...
    Widget buildProfileApp({
      ...
      final router = GoRouter(
        routes: [
          ...
          GoRoute(path: RoutePaths.outbox, builder: (_, _) => const Scaffold(body: Text('OutboxScreen'))),
          GoRoute(path: RoutePaths.anonymityGuide, builder: (_, _) => const AnonymityGuideScreen()),
        ],
      );
    ...
    testWidgets('ProfileScreen renders user avatar/mask icon, anon_id chip, and stats card for authenticated user', (WidgetTester tester) async {
      ...
      expect(find.textContaining(RegExp(r'Verified', caseSensitive: false)), findsAtLeastNWidgets(1));
    });
